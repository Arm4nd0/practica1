<?php
    class User
    {
        ...
        public function ConsultaEspecific($id)
        {
            ...
            $consulta = mysql_query($sql01) or die ("Error de busqueda 1");
            echo "<div class=table-resposive align=center>";
            echo "<table class=\*table table-striped\ border=1>";
            echo "<tr><td colspan=5 align=center>Resultado de Busqueda</td></tr>";
            echo"<tr><td> No.</td><td> Nombre(s) </td><td> Apellido Paterno </td><td> Apellido Materno </td><td> Nivel </td>";
            while($field = mysql_fetch_array($consulta))
            {
                $this->IdPersona = $field['IdPersona'];
                $this->Nombre = $field['nombre'];
        ...
        public function UpdateDatos($nombre,$apellidoPaterno,$apellidoMaterno,$nivel,$estatus,$idd)
        {
            ...
            $execute = mysql_query($update01) or die ("Error al modificar registro");
            echo "Modifico el registro $idd";
        }
        public function DeleteReg($id)
        {
            $delete01 = "DELETE FROM personascontrol WHERE IdPersona = $id";
            $execute = mysql_query($delete01) or die ("Error al eliminar registro");
            echo "Ha eliminado el registro $id";
        }
    }
?>